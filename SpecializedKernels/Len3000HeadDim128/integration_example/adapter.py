"""Stock Alpamayo 1.5 adapter for ``integration_example`` (no ROS).

Uses this folder's pack-and-reuse diffusion path and a fixed-slot expert
cache. ROS must keep using ``Len3000HeadDim128.alpamayo_adapter``.
"""
...
def _preallocate_attn(
    ...
    *,
    ...
    if for_diffusion:
        ...
        attn._alpamayo_diff_sel = torch.empty(1, hq, 16, device=device, dtype=torch.int32)
        attn._alpamayo_pack_k = torch.empty(1, hq, N_PACK_PAD, HEAD_DIM, device=device, dtype=dtype)
        attn._alpamayo_pack_v = torch.empty(1, hq, N_PACK_PAD, HEAD_DIM, device=device, dtype=dtype)
        attn._alpamayo_sel_ready = False
        attn._alpamayo_pack_ready = False
        attn._alpamayo_route_refresh = True
    else:
...
class AlpamayoRoutedAdapter:
    ...
    def reset_diffusion_step(self) -> None:
        """Start a new Euler loop at step 0 (needed before CUDA-graph capture)."""
        self._diff_step = 0
        self._n_route = 0
        self._n_reuse = 0
        for attn in self.expert_attns:
    ...
    def wrap_prompt_cache(self, cache: Any, n_prompt: int, n_diff: int) -> DiffusionStaticCache:
        """Copy the VLM prefix into fixed slots. Reuses the same buffers (graph-safe)."""
        n_prompt, n_diff = int(n_prompt), int(n_diff)
        existing = getattr(self, "_static_cache", None)
        if (
            existing is not None
            and existing.n_prompt == n_prompt
            and existing.n_diff == n_diff
            and existing.layers[0].keys.shape[0] == cache.layers[0].keys.shape[0]
        ):
            existing.load_prompt(cache, n_prompt)
            self._n_prompt = n_prompt
            self.refresh_diffusion_from_cache(existing, n_prompt)
            return existing
        static = DiffusionStaticCache.from_vlm_cache(cache, n_prompt=n_prompt, n_diff=n_diff)
        self._n_prompt = n_prompt
        self.refresh_diffusion_from_cache(static, n_prompt)
...
def _route_every() -> int:
    raw = os.environ.get("HGA_DIFF_ROUTE_EVERY", "3")
    try:
        n = int(raw)
    except ValueError:
        n = 3
...
def _patch_expert_eager_refresh(adapter: AlpamayoRoutedAdapter) -> None:
    """Fill prefix means once; re-pack groups every N Euler steps; drop the 4D mask.

    The official expert 4D mask is all-attend on the no-generation path
    (offset == prefill length). Hugging Face still treats a 4D float mask
    as "must expand GQA and take the math SDPA kernel". HGA ignores that
    mask; passing ``None`` skips the mask factory entirely.
    """
    ...
    orig = expert.forward
    last_id = {"cache": None}
    adapter._diff_step = 0
    adapter._n_route = 0
    adapter._n_reuse = 0
    every = _route_every()

    def forward(*args, past_key_values=None, attention_mask=None, **kwargs):
        cache = past_key_values
        if cache is None:
            cache = kwargs.get("past_key_values")
        if cache is not None and hasattr(cache, "layers"):
            cid = id(cache)
            if last_id["cache"] != cid:
                n_prompt = getattr(adapter, "_n_prompt", None)
                if n_prompt is None:
                    try:
                        n_prompt = int(cache.get_seq_length())
                    except TypeError:
                        n_prompt = int(cache.get_seq_length(0))
                    if isinstance(cache, DiffusionStaticCache):
                        n_prompt = cache.n_prompt
                if not getattr(cache.layers[0], "selected_keys", None):
                    adapter.refresh_diffusion_from_cache(cache, int(n_prompt))
                last_id["cache"] = cid
                adapter._diff_step = 0
                adapter._n_route = 0
                adapter._n_reuse = 0
            refresh = (adapter._diff_step % every) == 0
            adapter._diff_step += 1
            if refresh:
                adapter._n_route += 1
            else:
                adapter._n_reuse += 1
            for attn in adapter.expert_attns:
                attn._alpamayo_route_refresh = refresh
        # HGA does not use the 4D prefix mask (no-gen is all-attend).
        kwargs.pop("attention_mask", None)
        return orig(*args, past_key_values=past_key_values, attention_mask=None, **kwargs)
...
def adapt_alpamayo(
    ...
    **_ignored: Any,
    ...
    vlm_hq = _cfg_int(text_cfg, "num_attention_heads", default=32)
    vlm_kvh = _cfg_int(text_cfg, "num_key_value_heads", default=8)
    # Expert overrides only some text-config fields (Alpamayo 1.5: 16/8, not 32/8).
    exp_hq = _cfg_int(expert_cfg, "num_attention_heads", default=vlm_hq)
    exp_kvh = _cfg_int(expert_cfg, "num_key_value_heads", default=vlm_kvh)
    n_diff = 64
    try:
    ...
    adapter._n_prompt = None

    for attn in adapter.vlm_attns:
        _preallocate_attn(attn, hq=vlm_hq, kvh=vlm_kvh, max_q=max_seq, device=device, dtype=dtype)
    for attn in adapter.expert_attns:
        _preallocate_attn(
            attn, hq=exp_hq, kvh=exp_kvh, max_q=max(n_diff, 64), device=device, dtype=dtype,
            for_diffusion=True,
        )
    ...
    _warmup_kernels(device, dtype, vlm_hq, vlm_kvh, exp_hq, exp_kvh, n_diff)
    if fp8_residuals:
        print("[stock-hga] fp8_residuals ignored on the 2-level path", flush=True)
    print(
        f"[stock-hga] 2-level {CHUNK}/{GROUP} topk_c={TOPK_C} topk_g={TOPK_G} "
        f"vlm_tokens={GROUP + TOPK_G * GROUP} "
        f"vlm_heads={vlm_hq}/{vlm_kvh} expert_heads={exp_hq}/{exp_kvh} "
        f"diff_route_every={_route_every()} (pack K/V, means once/sample) "
        f"[example kernel, not ROS]",
        flush=True,
    )
...
def _warmup_kernels(device, dtype, vlm_hq, vlm_kvh, exp_hq, exp_kvh, n_diff) -> None:
    from .attention import diffusion_cross_attention, vlm_prefill_attention

    s = 2688
    q = torch.empty(1, vlm_hq, s, HEAD_DIM, device=device, dtype=dtype)
    k = torch.empty(1, vlm_kvh, s, HEAD_DIM, device=device, dtype=dtype)
    v = torch.empty_like(k)
    vlm_prefill_attention(q, k, v)
    qd = torch.empty(1, exp_hq, n_diff, HEAD_DIM, device=device, dtype=dtype)
    kd = torch.empty(1, exp_kvh, s + n_diff, HEAD_DIM, device=device, dtype=dtype)
    vd = torch.empty_like(kd)
    sel = torch.empty(1, exp_hq, 16, device=device, dtype=torch.int32)
    pk = torch.empty(1, exp_hq, N_PACK_PAD, HEAD_DIM, device=device, dtype=dtype)
    pv = torch.empty_like(pk)
    diffusion_cross_attention(
        qd, kd, vd, n_prompt=s, sel=sel, pack_k=pk, pack_v=pv, reuse_route=False,
    )
    diffusion_cross_attention(
        qd, kd, vd, n_prompt=s, sel=sel, pack_k=pk, pack_v=pv, reuse_route=True,
    )
    torch.cuda.synchronize()
    print(
        f"[stock-hga] 2L kernels compiled "
        f"(S={s} Q={n_diff} vlm={vlm_hq}/{vlm_kvh} expert={exp_hq}/{exp_kvh})",
        flush=True,
    )
