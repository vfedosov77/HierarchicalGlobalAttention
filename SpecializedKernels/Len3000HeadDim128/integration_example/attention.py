"""Stock no-ROS 2L attention (route-reuse). Do not import from the ROS path.

Wraps ``integration_example/kernel.py``. ROS uses the parent
``Len3000HeadDim128/attention.py``.
"""
...
DEFAULT_TOPK = TOPK_C
VLM_ATTN_NAME = "alpamayo_vlm_routed_stock"
DIFFUSION_ATTN_NAME = "alpamayo_diffusion_routed_stock"
...
def diffusion_cross_attention(
    ...
    *,
    ...
    pack_k: Optional[torch.Tensor] = None,
    pack_v: Optional[torch.Tensor] = None,
    reuse_route: bool = False,
    **_ignored: Any,
) -> torch.Tensor:
    """64 expert queries attend 11 packed 16-groups of the prefix + all diffusion keys.

    Prefix ``chunk_k`` / ``group_k`` are never rebuilt here — pass prefilled
    tables. ``reuse_route=True`` skips routing **and** reuses the already
    packed prefix K/V in ``pack_k`` / ``pack_v``. That is the part the old
    index-only reuse missed: the expensive work is the scattered gather,
    not the top-k scores.
    """
    hq, kvh = _check_heads(q, k)
    ...
    if sel is None or sel.shape[0] != b or sel.shape[1] != hq or sel.shape[-1] < TOPK_G:
        sel = torch.empty(b, hq, 16, device=q.device, dtype=torch.int32)
    if (
        pack_k is None
        or pack_v is None
        or pack_k.shape[0] != b
        or pack_k.shape[1] != hq
        or pack_k.shape[2] < N_PACK_PAD
    ):
        pack_k = torch.empty(b, hq, N_PACK_PAD, HEAD_DIM, device=q.device, dtype=q.dtype)
        pack_v = torch.empty_like(pack_k)
    if not getattr(diffusion_cross_attention, "_logged", False):
        print(
            f"[stock-hga] 2L diffusion pack-and-reuse q={q.dtype} k={k.dtype} "
            f"q_len={q_len} n_prompt={n_prompt} chunks={nc} "
            f"hq={hq} kvh={kvh} topk_c={TOPK_C} topk_g={TOPK_G} "
            f"route_every={_route_every()}",
            flush=True,
        )
        diffusion_cross_attention._logged = True
    if reuse_route:
        diff_attend_packed(q, k, v, pack_k, pack_v, out, n_prompt, float(softmax_scale))
        return out
    # One launch: route, pack the 176 prefix tokens, gather-attend.
    _diff_hga2_vec_kernel[(b * hq,)](
        q, k, v, gk, ck, sel, pack_k, pack_v,
        out,
        q_b, q_h, q_s,
        k_b, k_h, k_ss,
        v_b, v_h, v_s,
...
def hf_diffusion_attention(
    ...
    **kwargs: Any,
    ...
    chunk_k = getattr(module, "_alpamayo_chunk_k", None)
    group_k = getattr(module, "_alpamayo_group_k", None)
    sel = getattr(module, "_alpamayo_diff_sel", None)
    pack_k = getattr(module, "_alpamayo_pack_k", None)
    pack_v = getattr(module, "_alpamayo_pack_v", None)
    reuse = (not getattr(module, "_alpamayo_route_refresh", True)) and getattr(
        module, "_alpamayo_pack_ready", False
    )
    ...
    result = diffusion_cross_attention(
        ...
        sel=sel, pack_k=pack_k, pack_v=pack_v,
        reuse_route=reuse,
    )
    if not reuse:
        module._alpamayo_pack_ready = True
        module._alpamayo_sel_ready = True
    if dest is not None:
        return dest, None
    return result, None
