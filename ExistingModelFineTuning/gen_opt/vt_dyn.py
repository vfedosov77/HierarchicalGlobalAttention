...
class GlobalAttention(nn.Module):
    ...
    vt_dyn: eager-optimized decode.  The branchless static decode path exists to
    be CUDA-graph capturable; with CUDA graphs disabled it only adds ops, so this
    variant uses the dynamic per-token path (operates over the actual number of
    closed chunks, fewer ops) which is faster in pure eager.
    """

    use_static_decode: bool = False
    fp32_precise: bool = False
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        if q_len == 1 and cache_position is not None and self.use_static_decode:
    ...
    # ------------------------------------------------------------------
    # No-cache path: pure-torch dense routed attention (training / prefill).
    # Verbatim port of the reference ._forward_dense, so teacher-forcing loss
    # is identical to the original.  No Triton: the win for big models comes
    # from the routing (sub-quadratic), not from a fused kernel.
    # ------------------------------------------------------------------
    ...
    def _forward_no_cache(
        ...
        # Repeat GQA kv-heads up to all query heads, then run dense routed attention.
        q = q_new
        k_rope = self._repeat_kv(k_new)
        k_raw = self._repeat_kv(k_raw_new)
        v = self._repeat_kv(v_new)
        out = self._forward_dense(q, k_rope, k_raw, v, cos, sin)
        return out, None
    ...
    def _forward_dense(
        ...
        group_k = self._rope_summary(
            raw=k_raw_groups, rope=k_rope_groups, mask=group_token_mask, reduce_dim=4,
            anchor_pos=group_middle, cos=cos, sin=sin, scale=self.group_kv_scale,
        )
        ...
        if Kc > 0:
            ...
            cand_k_groups = self._gather_chunks(group_k, top_chunk_idx)
        ...
        # ---- 3) exact current tokens; current-chunk group summaries ----
        ...
        probs = F.dropout(torch.softmax(common_scores.float(), dim=-1).to(dtype),
        ...
        return self.o_proj(out_seq.transpose(1, 2).contiguous().reshape(B, S, H * Dh))
    ...
    def _decode_static(self, q_t: torch.Tensor, state: _HGAState, pos: torch.Tensor) -> torch.Tensor:
        ...
        prev = n - 1
        cst = self._decode_const(device, MAXN)
        ar_n = cst["ar_n"][:MAXN]
        ar_m = cst["ar_m"]
        ar_c = cst["ar_c"]
        ...
        # GQA-aware score: avoid materializing a head-repeated chunk_k copy.
        qk = q_t.view(B, H // rep, rep, Dh)
        sc_ck = torch.einsum("bkrd,bkjd->bkrj", qk, state.chunk_k).reshape(B, H, MAXN) * scale
        ...
        if self.topk_groups > 0:
            ...
            pos_d = pos_d.unsqueeze(-1) + cst["ar_gs"].view(1, 1, 1, gs)
        ...
        sc_B = torch.einsum("bkrd,bkgd->bkrg", qk, gk_n).reshape(B, H, M) * scale
        ...
        ck_tok = k_cache.index_select(2, idx_c)                       # [B,KVH,C,Dh]
        cur_v = self._rep_heads(v_cache.index_select(2, idx_c), rep)
        sc_C = torch.einsum("bkrd,bktd->bkrt", qk, ck_tok).reshape(B, H, C) * scale
    ...
    def _gather_kvhead_chunks(self, tensor: torch.Tensor, chunk_idx: torch.Tensor) -> torch.Tensor:
        ...
        ones = [1] * len(idx_shape)
        b_idx = torch.arange(B, device=tensor.device).view(B, 1, *ones)
        kv_idx = self._decode_const(tensor.device)["head2kv"].view(1, H, *ones)
    ...
    def _gather_kv_tokens(self, kv: torch.Tensor, pos: torch.Tensor) -> torch.Tensor:
        ...
        b_idx = torch.arange(B, device=kv.device).view(B, 1, 1)
        kv_idx = self._decode_const(kv.device)["head2kv"].view(1, H, 1)
    ...
    def _gather_group_values(
        ...
        ar_gs = self._decode_const(v_cache.device)["ar_gs"]
        pos = pos.unsqueeze(-1) + ar_gs.view(*([1] * pos.ndim), self.group_size)
    ...
    def _rotary_for_positions(self, pos: torch.Tensor, like: torch.Tensor) -> CosSin:
        device = like.device
        inv_freq = self._decode_const(device)["inv_freq"]
    ...
    def _decode_const(self, device: torch.device, min_chunks: int = 0) -> Dict[str, torch.Tensor]:
        """Lazily build & cache the device-resident constants the decode hot path
        rebuilt on every call in the reference (arange/mask/inv_freq).  These never
        depend on the runtime token position, so caching is exact and removes the
        bulk of the per-step CPU launch overhead."""
        cache = self.__dict__.get("_const_cache")
        if cache is None:
            cache = {}
            self.__dict__["_const_cache"] = cache
        key = str(device)
        c = cache.get(key)
        if c is None:
            half = self.head_dim // 2
            cutoff = self._mixed_cutoff_pair()
            pair = torch.arange(half, device=device) < cutoff
            c = {
                "inv_freq": 1.0 / (self.theta ** (torch.arange(half, device=device, dtype=torch.float32) / half)),
                "mix_mask": torch.cat([pair, pair], dim=0),
                "ar_m": torch.arange(self.groups_per_chunk, device=device),
                "ar_c": torch.arange(self.chunk_size, device=device),
                "ar_gs": torch.arange(self.group_size, device=device),
                "head2kv": torch.arange(self.nhead, device=device) // self.num_key_value_groups,
                "ar_n": torch.arange(max(1, min_chunks), device=device),
            }
            cache[key] = c
        if min_chunks > c["ar_n"].numel():
            c["ar_n"] = torch.arange(min_chunks, device=device)
        return c
    ...
    def _mix_tokenwise_and_anchor(self, tokenwise: torch.Tensor, anchor: torch.Tensor) -> torch.Tensor:
        mask = self._decode_const(tokenwise.device)["mix_mask"]
