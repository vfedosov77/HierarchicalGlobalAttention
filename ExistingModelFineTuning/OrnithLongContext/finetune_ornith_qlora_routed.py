"""QLoRA/LoRA fine-tune **Ornith-1.0-9B** (Qwen3.5 hybrid) with routed sparse attention active
during training — the Ornith analogue of ``finetune_qwen06b_qlora_routed``.

Ornith is a Qwen3.5 hybrid: only the 8 ``full_attention`` layers (idx 3,7,…,31) carry O(S) KV;
the 24 ``linear_attention`` (GatedDeltaNet) layers are untouched.  ``replace_ornith_attention_with_router``
swaps every ``Qwen3_5Attention`` (the full-attn layers only) for a drop-in
...
Run from the repo root::
    ...
    python -m ExistingModelFineTuning.OrnithLongContext.finetune_ornith_qlora_routed --smoke
    python -m ExistingModelFineTuning.OrnithLongContext.finetune_ornith_qlora_routed \
        --corpus pg19 --seq-len 2048 --epochs 3
...
# Aliased to the Qwen names so the rest of this file (ported 1:1 from the Qwen trainer) is unchanged.
try:
    from .ornith_routed_attention import (  # type: ignore
        OrnithRoutedAttention as QwenRoutedAttention,
        replace_ornith_attention_with_router as replace_qwen_attention_with_router,
        restore_ornith_attention as restore_original_attention,
...
def attention_mode(model: Any, knobs: Dict[str, Any], *, routed: bool, cache_location: str = "vram"):
    ...
    *same trained weights* are measured under both attentions and optimizer references stay valid.
    ...
    was_routed = any(isinstance(layer.self_attn, QwenRoutedAttention) for layer in _iter_ornith_attention_layers(base))
...
def load_routed_base(args, compute_dtype: torch.dtype):
    ...
    """
    # Ornith is a multimodal (image-text-to-text) Qwen3.5 hybrid; load the generative class so the
    # text stack + .generate() stay intact.  ``eager`` is the validated Ornith attention path
    # (see try_ornith / ORNITH_HGA.md §9); the routed full-attn layers use the exact-token path
    # regardless, so this only governs the dense-mode full-attn and the GatedDeltaNet layers.
    # ponytail: sdpa may be faster for the dense baseline but is unverified on the Qwen3.5 hybrid.
    ...
    if getattr(args, "quantization", "nf4") == "nf4":
        ...
        kwargs["quantization_config"] = BitsAndBytesConfig(
            ...
            # Keep lm_head / norms out of 4-bit (quantizing them hurts quality for little gain).
            llm_int8_skip_modules=["lm_head", "norm"],
        )
    model = AutoModelForImageTextToText.from_pretrained(args.model, **kwargs)
    model.config.use_cache = False
    # Multimodal wrapper: the decoder reads its own text config, so disable the KV cache there too.
    _tcfg = model.config.get_text_config() if hasattr(model.config, "get_text_config") else model.config
    _tcfg.use_cache = False

    # Text-only fine-tuning never invokes the vision tower (Qwen3_5Model.forward only calls it when
    # pixel_values is passed); free it to reclaim VRAM — the 9B body + vision tower otherwise
    # overflow a 16GB card during the routed forward assembly.  ponytail: this makes the loaded
    # model text-only; drop these lines to keep image support (needs a bigger card).
    _mm = getattr(model, "model", None)
    if _mm is not None and getattr(_mm, "visual", None) is not None:
        _mm.visual = None
        gc.collect()
        torch.cuda.empty_cache()
...
def _text_lora_targets(model: Any) -> List[str]:
    """Full module names of the LoRA targets under the text stack only.

    Ornith is multimodal; its vision encoder shares proj names (``q_proj`` …), so bare-suffix
    matching would attach dead LoRA adapters there (wasted VRAM on a 16GB card).  Restrict to
    modules whose path runs through ``language_model``; fall back to bare suffixes for a plain
    text model (the synthetic test config has no ``language_model`` wrapper).
    """
    names = [n for n, _ in model.named_modules()
             if "language_model" in n and n.rsplit(".", 1)[-1] in LORA_TARGETS]
    return names or LORA_TARGETS
...
def build_model(args, compute_dtype: torch.dtype):
    ...
    if getattr(args, "quantization", "nf4") == "nf4":
        ...
        # Non-quantized (bf16) path: no k-bit prep; still enable gradient checkpointing + input
        # grads so the frozen bf16 base backprops into the LoRA adapters (Target B, RTX 5090).
    ...
    lora_cfg = LoraConfig(
        ...
        target_modules=_text_lora_targets(model),
        bias="none",
        task_type="CAUSAL_LM",
    )
...
def smoke(args) -> None:
    ...
    # Ornith wraps only the full_attention layers (8 of 32), not every decoder layer.
    _sbase = model.get_base_model() if hasattr(model, "get_base_model") else model
    n_full = sum(1 for _ in _iter_ornith_attention_layers(_sbase))
    expected = 0 if args.attn_mode == "dense" else n_full
    ...
    assert all(
        ...
        for layer in _iter_ornith_attention_layers(base)
...
def parse_args(argv=None):
    ...
    p.add_argument("--model", default="deepreinforce-ai/Ornith-1.0-9B")
    ...
    p.add_argument("--quantization", choices=("nf4", "none"), default="nf4",
                   help="base-weight precision: 'nf4' = 4-bit QLoRA (Target A, 16GB Turing; default) "
                        "or 'none' = non-quantized LoRA (Target B, RTX 5090 32GB — pair with bf16 by "
                        "omitting --fp16). 'none' skips k-bit prep and BitsAndBytesConfig.")
