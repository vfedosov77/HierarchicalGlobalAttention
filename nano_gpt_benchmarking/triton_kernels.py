...
def linear_relu_square(a, b, aux=None, a_f8=None, b_f8=None, dequant_scale_ptr=None):
    ...
    BLOCK_SIZE_M = 128
    # BLOCK_SIZE_N lowered 256 -> 128 and num_stages 4/3 -> 3 so the kernel's shared-memory
    # footprint fits consumer Blackwell (sm_120, e.g. RTX 5090: ~99 KB/block). The original
    # H100-tuned config (BLOCK_N=256, num_stages=4 fwd / 3 bwd) needs ~180 KB and fails to launch
    # here with "out of resource: shared memory". Tiling/pipeline depth do not change the result,
    # only occupancy and speed. The datacenter-GPU config is preserved in triton_kernels_h100_backup.py.
    BLOCK_SIZE_N = 128
    ...
    double grad_scale_param)
{
  ...
  for (int i = 0; i < NUM_LOADS; i++) {
    ...
    if (i < NUM_FULL_LOADS || idx < VOCAB_SIZE) {
      ...
      result = *(__nv_bfloat168*)(&block_logit_ptr[idx]);
      #pragma unroll
      for (int k = 0; k < VEC_WIDTH; k++) {
        float tmp = __bfloat162float(result[k]);
    ...
    for (int k = 0; k < n_predict; k++) {
      ...
      if (target_idx < batch_size) {
        ...
        if (target >= 0 && target < VOCAB_SIZE) {
          float target_logit = __bfloat162float(block_logit_ptr[target]);
          float z_target = A * sigmoid(target_logit * inv_C + B_div_C);
  ...
  for (int i = 0; i < NUM_LOADS; i++) {
    ...
    if (i < NUM_FULL_LOADS || idx < VOCAB_SIZE) {
      __nv_bfloat168 logits_vec = *(__nv_bfloat168*)(&block_logit_ptr[idx]);
      #pragma unroll
      for (int j = 0; j < VEC_WIDTH; j++) {
        float logit = __bfloat162float(logits_vec[j]);
        float sigmoid_u = sigmoid(logit * inv_C + B_div_C);
        float z = A * sigmoid_u;
  ...
  if (threadIdx.x < n_predict && blockIdx.x + threadIdx.x < batch_size) {
    ...
    if (target >= 0 && target < VOCAB_SIZE) {
      float target_logit = __bfloat162float(block_logit_ptr[target]);
      float sigmoid_u = sigmoid(target_logit * inv_C + B_div_C);
      float z = A * sigmoid_u;
      float p = __expf(z - lse);
...
try:
    ...
    # This raw CUDA kernel targets sm_120 (fp8_e5m2). It is only needed by
    # FusedSoftcappedCrossEntropy; callers on unsupported hardware must avoid that path.
    import warnings as _warnings
    _warnings.warn(f"ce_fwd_bwd_kernel unavailable on this GPU: {_e}")
...
def ce_fwd_bwd(
    ...
    if ce_fwd_bwd_kernel is None:
        raise RuntimeError("ce_fwd_bwd_kernel is unavailable; FusedSoftcappedCrossEntropy requires an sm_120 CUDA build")
