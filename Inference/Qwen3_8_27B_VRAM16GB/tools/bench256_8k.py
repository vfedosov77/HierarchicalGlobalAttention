...
Examples:
    ...
    python3 tools/bench256_8k.py run --local
...
GEN_TOKENS = 64
# Turing1Qwen38Profile: K=2 MTP proposals + target token.
MTP_K = 2
VERIFY_WIDTH = 3
# 7993 prompt + 64 generated tokens fit with headroom for special tokens.
CTX = 8192
# Turing1Qwen38Profile: 8 prefill exchange pairs, 2 remain streamed in verify.
...
def rsync_to_turing1(host: str, remote_dir: str) -> None:
    ...
    cmd = [
        ...
        "--exclude", ".git",
        "--exclude", "/build",
        "--exclude", "/Inference/Qwen3_8_27B_VRAM16GB",
...
def remote_prep_parts(qwen: str) -> list[str]:
    return [
        f"cd {qwen}",
        "chmod +x scripts/*.sh tools/bench256_8k.py",
...
def fetch_remote_json(host: str, remote_json: str, local_json: Path) -> Optional[dict[str, Any]]:
    ...
    fetch_remote_file(
        host, f"/tmp/{ARTIFACT_STEM}.log", Path(f"/tmp/{ARTIFACT_STEM}.log")
    )
    if not ok:
...
def reprint_measure(host: str, args: argparse.Namespace, streamed: str, rc: int) -> int:
    record = fetch_remote_json(host, f"/tmp/{ARTIFACT_STEM}.json", Path(args.json))
    if record is None:
    ...
    local_log = Path(f"/tmp/{ARTIFACT_STEM}.log")
    dump: dict[str, Any] = {}
...
def deploy_and_maybe_run(args: argparse.Namespace, also_run: bool) -> int:
    ...
    remote_json = f"/tmp/{ARTIFACT_STEM}.json"
...
"""
    ...
    assert "tools/bench256_8k.py" in remote_prep_parts("~/HGA")[1]
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description=(
            "Deploy to turing1 and measure default-packing "
            "7993-token prefill with ubatch 768 + 64-token generate."
        )
    )
    p.add_argument(
        "command",
        ...
        help=(
            "deploy-and-run (default off turing1), "
            "run (default on turing1), or deploy only"
        ),
    )
    ...
    p.add_argument(
        "--local",
        action="store_true",
        help="Already on turing1; do not SSH",
    )
    p.add_argument(
        "--no-build",
        action="store_true",
    ...
    p.add_argument(
        ...
        help="Prompt path written on turing1 before the run",
    )
    p.add_argument(
        "--timeout",
        type=int,
