...
def load_bench():
    spec = importlib.util.spec_from_file_location("hga_vram16_bench", BENCH_PATH)
...
"""
    ...
    def test_measure_line_round_trip(self) -> None:
        ...
        parsed = self.bench.parse_measure_line("noise\n" + line + "\nmore")
    ...
    def test_cli_is_local_only(self) -> None:
        parser = self.bench.build_parser()
        with self.assertRaises(SystemExit), redirect_stderr(StringIO()):
            parser.parse_args(["deploy-and-run"])
        with self.assertRaises(SystemExit), redirect_stderr(StringIO()):
            parser.parse_args(["--host", "nobody@example"])
        args = parser.parse_args(["run", "--local"])
        self.assertEqual(args.command, "run")
        self.assertTrue(args.local)
        self.assertFalse(hasattr(args, "host"))
        self.assertFalse(hasattr(args, "remote_dir"))
        self.assertFalse(hasattr(self.bench, "askpass_env"))
        self.assertFalse(hasattr(self.bench, "read_turing1_password"))
        self.assertFalse(hasattr(self.bench, "rsync_to_turing1"))
