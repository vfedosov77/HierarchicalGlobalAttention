...
llama_model_qwen35::graph::graph(const llama_model & model, const llm_graph_params & params) :
    ...
    hga_pin_gpu_prefill_probe(sched, inpL, cparams.hga_phase);
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        cb(cur, "attn_norm", il);
        hga_pin_gpu_prefill_probe(sched, cur, cparams.hga_phase);
        ...
        hga_pin_gpu_prefill_probe(sched, attn_post_norm, cparams.hga_phase);
    ...
    hga_pin_gpu_prefill_probe(sched, cur, cparams.hga_phase);
    /* GPU lm_head: leave logits on CUDA. Pin CPU only if the weight is host. */
...
std::pair<ggml_tensor *, ggml_tensor *> llama_model_qwen35::graph::build_qkvz(
    ...
    hga_pin_gpu_prefill_probe(sched, qkv_mixed, cparams.hga_phase);

    ggml_tensor * z = build_lora_mm(model.layers[il].wqkv_gate, input, model.layers[il].wqkv_gate_s);
    cb(z, "z", il);
    hga_pin_gpu_prefill_probe(sched, z, cparams.hga_phase);
...
ggml_tensor * llama_model_qwen35::graph::build_layer_attn(
    ...
    hga_pin_gpu_prefill_probe(sched, Qcur_full, cparams.hga_phase);

    ggml_tensor * Qcur = ggml_view_3d(ctx0, Qcur_full, n_embd_head, n_head, n_tokens,
        ggml_element_size(Qcur_full) * n_embd_head * 2,
        ggml_element_size(Qcur_full) * n_embd_head * 2 * n_head, 0);
    cb(Qcur, "Qcur_reshaped", il);
    hga_pin_gpu_prefill_probe(sched, Qcur, cparams.hga_phase);
    /* VERIFY copies the DECODE graph (CPU q/k-norm + RoPE). n_tokens > 1
     * cannot D2H the strided Q+gate view on sm_70 — materialize first. */
    ...
    /* build_norm returns MUL(RMS_NORM(Q), weight).  Keep the GPU RMS root so
    ...
    cb(Kcur, "Kcur", il);
    hga_pin_gpu_prefill_probe(sched, Kcur, cparams.hga_phase);
    ggml_tensor * Vcur = build_lora_mm(model.layers[il].wv, cur, model.layers[il].wv_s);
    cb(Vcur, "Vcur", il);
    hga_pin_gpu_prefill_probe(sched, Vcur, cparams.hga_phase);

    /* Q RMS-normalization and K/V projection are independent CUDA work.
    ...
    /* Same packing as prefill: all attn linears on CUDA. HGA stays CPU. */
    ...
    hga_pin_gpu_prefill_probe(sched, gate, cparams.hga_phase);
    ...
    /* Constructor only: park the dense chain on mmap. Prefill keeps rope on
     * CUDA with q-norm. Decode leaves q/k-norm+rope on CPU (HGA dataflow). */
    if (!cparams.offload_kqv && !hga_gpu_pack(cparams.hga_phase)) {
        ggml_backend_sched_set_tensor_backend(sched, Qcur, backend_cpu);
        ggml_backend_sched_set_tensor_backend(sched, Kcur, backend_cpu);
        ggml_backend_sched_set_tensor_backend(sched, Vcur, backend_cpu);
        ggml_backend_sched_set_tensor_backend(sched, Kraw, backend_cpu);
    ...
    hga_pin_gpu_prefill_probe(sched, cur, cparams.hga_phase);
    /* Explicit H2D of HGA output so o_proj stays on CUDA (same in both modes). */
    if (cparams.hga_enabled && hga_gpu_pack(cparams.hga_phase)) {
        cur = hga_copy_to_gpu(this, cur, "hga_attn_gpu");
    }

    ggml_tensor * gate_sigmoid = ggml_sigmoid(ctx0, gate);
    cb(gate_sigmoid, "gate_sigmoid", il);
    hga_pin_gpu_prefill_probe(sched, gate_sigmoid, cparams.hga_phase);
...
ggml_tensor * llama_model_qwen35::graph::build_layer_attn_linear(
    ...
    cb(beta, "beta", il);
    hga_pin_gpu_prefill_probe(sched, beta, cparams.hga_phase);
    ...
    cb(alpha, "alpha", il);
    hga_pin_gpu_prefill_probe(sched, alpha, cparams.hga_phase);

    ggml_tensor * alpha_biased   = ggml_add(ctx0, alpha, model.layers[il].ssm_dt);
    ggml_tensor * alpha_softplus = ggml_softplus(ctx0, alpha_biased);
    cb(alpha_softplus, "a_softplus", il);
    hga_pin_gpu_prefill_probe(sched, alpha_softplus, cparams.hga_phase);
    ...
    hga_pin_gpu_prefill_probe(sched, state, cparams.hga_phase);

    ggml_tensor * conv_output_proper = ggml_ssm_conv(ctx0, conv_input, conv_kernel);
    cb(conv_output_proper, "conv_output_raw", il);
    hga_pin_gpu_prefill_probe(sched, conv_output_proper, cparams.hga_phase);

    ggml_tensor * conv_output_silu = ggml_silu(ctx0, conv_output_proper);
    cb(conv_output_silu, "conv_output_silu", il);
    hga_pin_gpu_prefill_probe(sched, conv_output_silu, cparams.hga_phase);
    ...
    hga_pin_gpu_prefill_probe(sched, q_conv, cparams.hga_phase);
    cb(k_conv, "k_conv", il);
    hga_pin_gpu_prefill_probe(sched, k_conv, cparams.hga_phase);
    cb(v_conv, "v_conv", il);
    hga_pin_gpu_prefill_probe(sched, v_conv, cparams.hga_phase);
    ...
    hga_pin_gpu_prefill_probe(sched, final_output, cparams.hga_phase);
...
llama_model_qwen35::graph_mtp::graph_mtp(const llama_model & model, const llm_graph_params & params)
    ...
    hga_pin_gpu_prefill_probe(sched, tok_embd, cparams.hga_phase);
    ...
    /* MTP has no weight-swap, so pick the same HGA packing the trunk uses:
     * large catch-up = PREFILL (q-norm CUDA, implicit D2H); draft/verify-sized
     * batches = DECODE (CPU q/k-norm, 24 KiB D2H). */
    if (cparams.hga_runtime) {
        const_cast<llama_cparams &>(cparams).hga_phase =
                n_tokens > 8 ? HGA_SWAP_PREFILL : HGA_SWAP_DECODE;
    }

    /* In HGA mode this draft block has its own routed KV session, so it must
     * not instantiate the normal full-context K/V graph inputs.  Keeping a
     * zero-layer llama KV memory object still provides sequence bookkeeping
     * to speculative decoding, but no duplicate per-token K/V tensors. */
    ...
    hga_pin_gpu_prefill_probe(sched, h_norm, cparams.hga_phase);
    hga_pin_gpu_pack(sched, h_norm, cparams.hga_phase);

    ggml_tensor * e_norm = build_norm(tok_embd, layer.nextn.enorm, nullptr, LLM_NORM_RMS, il);
    cb(e_norm, "mtp_enorm", il);
    hga_pin_gpu_prefill_probe(sched, e_norm, cparams.hga_phase);
    hga_pin_gpu_pack(sched, e_norm, cparams.hga_phase);

    ggml_tensor * concat = ggml_concat(ctx0, e_norm, h_norm, /*dim=*/ 0);
    cb(concat, "mtp_concat", il);
    hga_pin_gpu_prefill_probe(sched, concat, cparams.hga_phase);
